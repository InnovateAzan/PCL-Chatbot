...
class ItTicketService:
    ...
    async def get_user_tickets(self, current_user: AuthenticatedUser) -> list[dict[str, Any]]:
        log_event(logger, "ticket_lookup_started", operation="all", user=_safe_user(current_user))
        tickets = await self._owned_tickets(current_user)
        log_event(logger, "ticket_lookup_completed", operation="all", result_count=len(tickets))
        return [ticket.to_dict() for ticket in tickets]

    async def get_open_tickets(self, current_user: AuthenticatedUser) -> list[dict[str, Any]]:
        log_event(logger, "ticket_lookup_started", operation="open", user=_safe_user(current_user))
        tickets = [
            ticket.to_dict()
            for ticket in await self._owned_tickets(current_user)
            if _normalize_status(ticket.status) in OPEN_STATUSES
        ]
        log_event(logger, "ticket_lookup_completed", operation="open", result_count=len(tickets))
        return tickets

    async def get_closed_tickets(self, current_user: AuthenticatedUser) -> list[dict[str, Any]]:
        log_event(logger, "ticket_lookup_started", operation="closed", user=_safe_user(current_user))
        tickets = [
            ticket.to_dict()
            for ticket in await self._owned_tickets(current_user)
            if _normalize_status(ticket.status) in CLOSED_STATUSES
        ]
        log_event(logger, "ticket_lookup_completed", operation="closed", result_count=len(tickets))
        return tickets

    async def get_latest_ticket(self, current_user: AuthenticatedUser) -> dict[str, Any] | None:
        log_event(logger, "ticket_lookup_started", operation="latest", user=_safe_user(current_user))
        tickets = await self._owned_tickets(current_user)
        log_event(logger, "ticket_lookup_completed", operation="latest", result_count=1 if tickets else 0)
    ...
    async def get_ticket_summary(self, current_user: AuthenticatedUser) -> dict[str, int]:
        log_event(logger, "ticket_lookup_started", operation="summary", user=_safe_user(current_user))
        tickets = await self._owned_tickets(current_user)
        summary = {
            "open": 0,
            "pending": 0,
        ...
        for ticket in tickets:
            ...
            if status in summary:
                summary[status] += 1
        log_event(logger, "ticket_lookup_completed", operation="summary", result_count=len(tickets))
        return summary
    ...
    async def _list_items(self) -> list[dict[str, Any]]:
        ...
        try:
            items = await client.get_list_items(site_id, list_id, top=200)
            log_event(logger, "ticket_graph_items_loaded", graph_item_count=len(items))
            return items
        except GraphClientError as exc:
            status_code = exc.detail.status_code
            if status_code in {401, 403}:
                raise ItTicketPermissionError("graph_permission_denied") from exc
            if status_code == 404:
    ...
    async def _resolved_site_id(self) -> str:
        ...
        if configured:
            ...
            log_event(logger, "ticket_site_resolved", configured=True, site_resolved=True)
            return configured
        log_event(logger, "ticket_site_resolution_started", site_url_configured=bool(self.settings.onedesk_site_url))
        site = await (await self._sharepoint_client()).resolve_site(self.settings.onedesk_site_url)
        site_id = str(site.get("id") or "")
        if not site_id:
            log_event(logger, "ticket_site_resolution_failed", level=logging.ERROR, site_resolved=False)
            raise ItTicketConfigurationError("IT Service Desk site could not be resolved.")
        self._site_id = site_id
        log_event(logger, "ticket_site_resolved", configured=False, site_resolved=True)
        return site_id
    ...
    async def _resolved_list_id(self, site_id: str) -> str:
        ...
        if configured:
            ...
            log_event(logger, "ticket_list_resolved", configured=True, list_resolved=True)
            return configured
        log_event(logger, "ticket_list_resolution_started", list_title=self.settings.effective_it_service_desk_list_title)
        ...
        if not list_id:
            log_event(logger, "ticket_list_resolution_failed", level=logging.ERROR, list_resolved=False)
            raise ItTicketConfigurationError("IT Service Desk list could not be resolved.")
        self._list_id = list_id
        log_event(logger, "ticket_list_resolved", configured=False, list_resolved=True)
        return list_id

    async def _sharepoint_client(self) -> SharePointClient:
        if self._client is not None:
            return self._client
        format_error = token_format_error(self.user_access_token)
        if format_error:
            log_auth_stage(
...
def _safe_user(current_user: AuthenticatedUser) -> dict[str, str | None]:
    return {
